...
def train(dataset, val_dataset, v, start_epoch=0):
    ...
    optimizer = optim.Adam(model.parameters(),lr=config.learning_rate,batch_size=config.batch_size,shuffle=True,collate_fn=collate_fn)
        ...
        for epoch in range(start_epoch, config.epochs):
            ...
            with tqdm(total=len(train_dataloader) // config.batch_size)\
                ...
                for batch, data in enumerate(tqdm(train_dataloader)):
                    ...
                    clip_grad_norm_(model.dencoder.parameters(), config.max_grad_norm)
                    ...
                    if (batch % 100) == 0:
                        ...
                        writer.add_scaler(f'average loss for epoch{epoch}',np.mean(batch_losses),global_step=batch)
