...
from utils import source2ids, outputids2words, Beam, timer
import jieba
...
class Predict():
    ...
    def __init__(self):
        ...
        self.model = Seq2seq(self.vocab, embedding_weights=None)
        self.stop_word = list(
            set([
    ...
    def greedy_search(self, encoder_input, max_sum_len, max_oovs):
        ...
        decoder_input_t = torch.ones(1) * self.vocab.SOS  # initialize with start token
        decoder_input_t = decoder_input_t.to(self.DEVICE, dtype=torch.int64)

        summary = [self.vocab.SOS]

        while int(decoder_input_t.tolist()[0]) != (self.vocab.EOS) and len(summary) < max_sum_len:  # as long as decoder input is different from end token: continue
            context_vector, attention_weights = self.model.attention(decoder_states, encoder_output)
            p_vocab, decoder_states = self.model.decoder(decoder_input_t.unsqueeze(1),
                                                         decoder_states,
                                                         encoder_output,
                                                         context_vector)
            ...
            oov_token = torch.full(decoder_input_t.shape, self.vocab.UNK).long().to(self.DEVICE)
            decoder_input_t = torch.where(decoder_input_t > len(self.vocab) - 1, oov_token, decoder_input_t).to(self.DEVICE)
...
#     @timer('best k')
    def best_k(self, beam, k, encoder_output, x_padding_masks):
        ...
        Args:
            ...
            x_padding_masks (Tensor): The padding masks for the input sequences.
        ...
        decoder_input_t = torch.tensor(beam.tokens[-1]).unsqueeze(0).unsqueeze(1).to(self.DEVICE)
        oov_token = torch.full(decoder_input_t.shape, self.vocab.UNK).long().to(self.DEVICE)
        decoder_input_t = torch.where(decoder_input_t > len(self.vocab) - 1, oov_token, decoder_input_t)
...
#                 log_probs[token] = log_probs[token] * 1.5 
    ...
    def beam_search(self, encoder_input, max_sum_len, beam_width, max_oovs, x_padding_masks):
        ...
        # get the beam size and create a list for stroing current candidates and a list for completed hypothesis
        k = beam_width
        curr, completed = [beam], []
        ...
        for i in range(max_sum_len):
            ...
            candidates = []
            for beam in curr:
                candidates += self.best_k(beam, k, encoder_output, x_padding_masks)
            curr = sorted(candidates, key=lambda x:x.seq_score(), reverse=True)[:k]
            for beam in curr:
            ...
            if len(completed) == k:
                break
        # When there are not engouh completed hypotheses, take whatever when have in current best k as the final candidates.
    ...
    def predict(self, text, tokenize=True, beam_search=True):
        ...
        Args:
            ...
            tokenize (bool, optional): Whether to do tokenize or not. Defaults to True.
            beam_search (bool, optional): Whether to use beam search or not. Defaults to True (means using greedy search).
        ...
        return outputids2words(summary, oov, self.vocab).replace('<SOS>', '').replace('<EOS>', '').strip()
