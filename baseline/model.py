...
@LastEditTime: 2020-07-13 20:10:14
@LastEditors: Please set LastEditors
@Description: In User Settings Edit
@FilePath: /baseline/model.py
'''
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        Returns:
            output (Tensor): The output of lstm with shape (batch_size, seq_len, 2 * hidden_units)
            hidden (tuple): The hidden states of lstm (h_n, c_n). Each with shape (2, batch_size, hidden_units)
        """
...
class Attention(nn.Module):
    ...
    def forward(self, decoder_states, encoder_output, x_padding_masks):
        ...
        attention_weights = F.softmax(score, dim=1).squeeze(2) * x_padding_masks # (batch_size, seq_length)
...
class Decoder(nn.Module):
    ...
    def forward(self, decoder_input, decoder_states, encoder_output,
        ...
        Args:
            ...
            encoder_output (Tensor): Thw output from the encoder of shape (batch_size, seq_length, 2*hidden_units).
        ...
        concat_vector = torch.cat(
            [decoder_output.view(-1, config.hidden_size), context_vector], dim=-1) # (batch_size, 3*hidden_units)

        # calculate vocabulary distribution
        FC1_out = self.W1(concat_vector) # (batch_size, hidden_units)
        FC2_out = self.W2(FC1_out) # (batch_size, vocab_size)
        p_vocab = F.softmax(FC2_out, dim=1) # (batch_size, vocab_size)
...
class ReduceState(nn.Module):
    ...
    we add this module to reduce the hidden states output by the encoder (merge two directions) before 
    input the hidden states into the decoder.
    """
    def __init__(self):
    ...
    def forward(self, hidden):
        ...
        Args:
            hidden (tuple): Hidden states of encoder, each with shape (2, batch_size, hidden_units).

        Returns:
            tuple: Reduced hidden states, each with shape (1, batch_size, hidden_units).
        """        
        h, c = hidden
...
class Seq2seq(nn.Module):
    ...
    def forward(self, x, x_len, y, len_oovs, batch):
        ...
        encoder_output, encoder_states = self.encoder(x_copy, x_len)
